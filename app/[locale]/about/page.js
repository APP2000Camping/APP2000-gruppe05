...
export default function About({ params: { locale } }) {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      ...
      try {
        const response = await fetch('/api/getArticle?section=About');
        if (response.ok) {
  ...
  const handleSubmit = async (index) => {
    ...
    const article = { ...articles[index], section: 'About' };
  ...
  const handleDeleteArticle = async (index) => {
    ...
    try {
      const response = await fetch(`/api/delArticle?id=${encodeURIComponent(article.id)}&section=About`, {
        method: 'DELETE',
      });
  ...
  const handleAddArticle = () => {
    const newArticle = { title: '', content: '', section: 'About' };
